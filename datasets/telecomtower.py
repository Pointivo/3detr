...
from utils.box_util import flip_axis_to_camera_tensor, get_3d_box_batch_tensor, flip_axis_to_camera_np, \
    get_3d_box_batch_np

MEAN_COLOR_RGB = np.array([0.5, 0.5, 0.5])  # sunrgbd color is in 0~1 (not relevant if use_rgb=False)
...
class TelecomTowerDatasetConfig(object):
    ...
    def angle2class(self, angle):
        ...
        class_id = int(shifted_angle / angle_per_class)  # the angle is shifted by half class-size due to int (truncate)
        residual_angle = shifted_angle - (class_id * angle_per_class + angle_per_class / 2)
        return class_id, residual_angle  # residual \in [-angle_per_class, angle_per_class]
    ...
    def my_compute_box_3d(center, size, heading_angle):
        ...
        l, w, h = size
        # applying rotation to box while at origin
        x_corners = [-l, l, l, -l, -l, l, l, -l]
        y_corners = [w, w, -w, -w, w, w, -w, -w]
        z_corners = [h, h, h, h, -h, -h, -h, -h]
        corners_3d = np.dot(R, np.vstack([x_corners, y_corners, z_corners]))
        # translating bbox to `center`
        corners_3d[0, :] += center[0]
        corners_3d[1, :] += center[1]
        corners_3d[2, :] += center[2]
        return np.transpose(corners_3d)  # convert 3xN to Nx3 (N=8, 3:XYZ)


class TelecomTowerDetectionDataset(Dataset):
    def __init__(
            self,
            dataset_config,
            split_set="train",
            root_dir=None,
            num_points=NUM_POINTS_SAMPLING,
            use_color=False,
            use_height=False,
            use_v1=True,
            augment=False,
    ...
    def __getitem__(self, idx):
        ...
        for i in range(bboxes.shape[0]):
            bbox = bboxes[i]
            # semantic_class = bbox[7]  # object class
            raw_angles[i] = bbox[6] % 2 * np.pi  # heading angle
            box3d_size = bbox[3:6] * 2  # L,W,H
            raw_sizes[i, :] = box3d_size
            angle_class, angle_residual = self.dataset_config.angle2class(bbox[6])  # from heading angle
            angle_classes[i] = angle_class
            angle_residuals[i] = angle_residual
            corners_3d = self.dataset_config.my_compute_box_3d(center=bbox[0:3], size=bbox[3:6], heading_angle=bbox[6])
            # compute axis aligned box from original bbox
            ...
            target_bbox = np.array(
                [
                    (xmin + xmax) / 2, (ymin + ymax) / 2, (zmin + zmax) / 2,    # center coords
                    xmax - xmin, ymax - ymin, zmax - zmin,                      # L, W, H
                ]
            )
...
class TelecomTowerPredictionDataset(Dataset):

    def __init__(self, path_to_data_dir: Path, point_cloud_suffix: str = "_pc.npz",
                 num_points: int = NUM_POINTS_SAMPLING, use_color: bool = False, use_height: bool = False):
        assert path_to_data_dir.exists(), f"Non-existent file/folder: {path_to_data_dir}"
        self.path_to_data_dir = path_to_data_dir
        self.point_cloud_suffix = point_cloud_suffix
        self.num_points = num_points
        self.use_color = use_color
        self.use_height = use_height
        self.point_clouds_in_dir = list(self.path_to_data_dir.glob(f"*{self.point_cloud_suffix}"))
        print(f"Found {len(self.point_clouds_in_dir)} point clouds (*{self.point_cloud_suffix})")

    def __len__(self):
        return len(self.point_clouds_in_dir)

    def __getitem__(self, idx: int, bboxes: bool = False):
        assert 0 <= idx < len(self.point_clouds_in_dir)
        path_to_point_cloud = self.point_clouds_in_dir[idx]
        point_cloud = np.load(str(path_to_point_cloud))["pc"]  # Nx6
        if not self.use_color:
            point_cloud = point_cloud[:, 0:3]
        else:
            assert point_cloud.shape[1] == 6
        ...
        # point cloud sub-sampling
        point_cloud, choices = pc_util.random_sampling(point_cloud, self.num_points, return_choices=True)

        ret_dict = {
            "scan_idx": np.array(idx).astype(np.int64),
            "point_cloud_filename": path_to_point_cloud.stem,
            "point_clouds": point_cloud.astype(np.float32),
            "point_cloud_dims_min": point_cloud.min(axis=0),
            "point_cloud_dims_max": point_cloud.max(axis=0),
        }
        if bboxes:  # load bounding boxes (ground truth)
            # raise NotImplementedError(f"Still working on it!")
            ret_dict.update({
                "bboxes": np.load(
                    str(
                        path_to_point_cloud.parent / (path_to_point_cloud.stem.replace("_pc", "_bbox") + ".npy")))
            })  # K,8
        return ret_dict
